...
#<START>
echo;echo -e "<START> GATK4 BAM QC +MULTI [FENIX]"
echo "Started: $(date)"
...
# BAM LIST format: 
## Sample_name \t mapped_BAMfile_1 \t ReadGroup_string_1
## Sample_name \t mapped_BAMfile_2 \t ReadGroup_string_2...

BAM_LIST=$1
OUTPUT_PATH=${2:-$PWD}
declare -a BAM_files_array

if [ -f "$BAM_LIST" ]; then
    echo "> Input file list: $BAM_LIST"
    echo "> Output folder: ${OUTPUT_PATH}"
    BAM_sample=$( head -n 1 "$BAM_LIST" | cut -f 1 )
    echo "> Sample name: ${OUTPUT_PATH}"
    BAM_base="${BAM_sample}.merged"
    FINAL_FILE="${OUTPUT_PATH}/${BAM_base}.rmdup.mqfilt.bqsr.bam"
else
    echo -e "<ERROR> GATK4 BAM QC CANCELLED. File not found: ${BAM_LIST}"
    exit 1
fi
##</INPUT>
...
# QC STEP 0-mod: Add ReadGroup
step0_add_readgroup() {
    local step_name="Step 0: ReadGroup Assignation (iterative)"
    local infile="$1"
    local rg_string="$2"
    local outfile="$3"
...
# QC STEP 0-MULTIPLEX: Runs ReadGroup using the 
step0_add_readgroup_multi() {
    local step_name="Step 0X: ReadGroup Assignation Multiplexer"
    local step_timestamp=$(date +%s)
...
step1_mark_duplicates_spark_multi() {
    ...
    gatk --java-options "-Xmx4G" MarkDuplicatesSpark \
        ...
        --verbosity ERROR \
        --output-shard-tmp-dir ${OUTPUT_PATH}/$RANDOM.parts/ \
...
step2_mapping_quality_filter() {
    ...
    local step_timestamp=$(date +%s)
    local infile="${OUTPUT_PATH}/${BAM_sample}.merged.rmdup.bam"
