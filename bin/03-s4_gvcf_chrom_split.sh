...
if [ -f "$GVCF_FILE" ]; then
    ...
    GVCF_prefix=${GVCF_name%%*.raw_variants.canon_chr.g.vcf.gz}
else
...
step4r_split_chroms_gvcf() {
    local step_name="Step 4(r): Split Chromosomes GVCFs"
    local infile="${GVCF_path}"
    local outfile="${OUTPUT_PATH}/chrom_gvcf/${GVCF_prefix}.raw_vars"
